...
import {
  autoCommit,
  collectHookCommands,
  collectStagedCommands,
  init,
  install,
  reAddStagedFiles
} from "@doremijs/igit-core";
...
async function start() {
  ...
  switch (command) {
    case "init":
      ...
      break;
    case "install":
      if (process.env.CI === "true" || options.includes("--skip-install")) {
        console.log(`${LOG_PREFIX}Skip install step`);
      } else {
        install();
      }
      break;
    case "run":
      if (options[0] === "pre-commit") {
        ...
        for (const command2 of commands) {
          runCommand(command2);
          if (command2.files) {
            files.push(...command2.files);
          }
        }
        if (files.length) {
          reAddStagedFiles(files);
        }
      }
